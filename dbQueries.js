...
const getAllOrders = (data, flag, callback) => {
    let sql;
    if (flag === 'restaurant') {
        sql = `SELECT * FROM order_tb WHERE r_id = ? ORDER BY o_datetime DESC`;
    } else {
        sql = `
...
const confirmOrder = (od_id, status, callback) => {
    ...
    sql = `UPDATE order_tb SET o_status = ? WHERE o_id = ?`;
    connection.query(sql, [newStatus, od_id], (err, results) => {
        ...
        connection.query(selectSql, [od_id], (err, results) => {
            ...
            callback({ message: "Success", data: results[0] });
        });
    });
};
...
export {getAllDishes, getAllRestaurants, addCustomer, addRestaurant, getUserDetails, 
    ...
    getRestaurant, getRestaurantwithFilter, addRestReview, addDishReview, getReviews, confirmOrder, getOrderDetails, getRestIdFromCart, connection};